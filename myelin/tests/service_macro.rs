//! Smoke test that `#[myelin::service]` expands to code that compiles.
//!
//! Exercises:
...
// `myelin` has no `serde` feature of its own; the macro emits
...
impl BlockOn for TokioBlockOn {
    fn block_on<F: core::future::Future>(&self, fut: F) -> F::Output {
        // The myelin BlockOn trait deliberately does not require Send; we use
